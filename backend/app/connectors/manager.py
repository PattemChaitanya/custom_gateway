    ...
    async def connect_connector(self, connector_id: int):
        ...
        try:
            if connector_type in ["postgresql", "mongodb"]:
                instance = create_database_connector(connector_type, connector.config)
            elif connector_type in ["redis", "kafka"]:
                instance = create_queue_connector(connector_type, connector.config)
            elif connector_type in ["s3", "azure"]:
                instance = create_storage_connector(connector_type, connector.config)
            else:
