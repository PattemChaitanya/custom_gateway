...
async def cleanup_old_logs(session: AsyncSession, retention_days: int = 30) -> int:
    ...
    """
    # Fallback backends can provide their own retention implementation.
    if hasattr(session, "cleanup_old_logs"):
        return await session.cleanup_old_logs(retention_days=retention_days)
...
async def get_log_statistics(session: AsyncSession) -> dict:
    ...
    # Fallback backends can provide direct statistics.
    if hasattr(session, "get_audit_log_statistics"):
        return await session.get_audit_log_statistics()
    ...
    logs_by_type_query = await session.execute(
        select(AuditLog.action, func.count(
            AuditLog.id)).group_by(AuditLog.action)
    )
