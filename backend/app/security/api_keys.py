...
class APIKeyManager:
    ...
    async def create_api_key(
        ...
        """Create a new API key and return the APIKey object with plain key."""
        # Generate key
        ...
        # Add expires_at if the column exists
        if hasattr(APIKey, 'expires_at'):
        ...
        # Attach the plain key as an attribute (temporary, not persisted)
        api_key.key = plain_key  # Override with plain key temporarily
        api_key.key_preview = f"{plain_key[:8]}..." if len(plain_key) > 8 else plain_key
        ...
        for key_obj in keys:
            if verify_api_key(plain_key, key_obj.key):
                ...
                if hasattr(key_obj, 'expires_at') and key_obj.expires_at:
                    ...
                    if now > expires_at:
                        logger.warning(f"Expired API key used: {key_obj.label}")
                        return None
                ...
                if hasattr(key_obj, 'last_used_at'):
                    await self.session.execute(
                        ...
                        .values(
                            ...
                            usage_count=(APIKey.usage_count + 1) if hasattr(APIKey, 'usage_count') else None
                        )
                    )
        ...
        return [
            {
                ...
                "last_used_at": key.last_used_at.isoformat() if hasattr(key, 'last_used_at') and key.last_used_at else None,
