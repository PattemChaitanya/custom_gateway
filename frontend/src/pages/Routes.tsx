...
export default function Routes() {
  ...
  const [configMode, setConfigMode] = useState<'resource' | 'json' | 'yaml' | 'terraform'>('resource');
  ...
  const rows = useMemo(() => {
    ...
    return source.filter((r) => r.resource.toLowerCase().includes(f) || r.method.toLowerCase().includes(f)).slice(0, showCount);
  ...
  function rowsToCanonical(rows: RouteRow[], apiName = 'MyAPI') {
    return {
      ...
      routes: rows.map((r) => ({
        ...
        integration: { type: r.integration?.toLowerCase().includes('http') ? 'http' : 'lambda', url: '' },
      })),
    };
  }
  ...
  function validateCanonical(obj: any): string[] {
    ...
    if (!Array.isArray(obj.routes)) errs.push('routes: must be an array');
      ...
      obj.routes.forEach((r: any, i: number) => {
        ...
        else if (!['API_KEY', 'JWT', 'NONE'].includes(String(r.auth).toUpperCase())) errs.push(`routes[${i}].auth: invalid`);
        if (!r.integration || !r.integration.type) errs.push(`routes[${i}].integration: type required`);
      });
    }
    return errs;
  }
  ...
  function applyTerraform() {
    ...
    const names = lines.filter((l) => l.startsWith('resource')).map((l) => l.split(' ')[1] ?? l);
    setValidationErrors([]);
    setCanonical({ apiVersion: 'v1', name: names[0] ?? 'tf-api', type: 'REST', routes: [] });
    setJsonText(JSON.stringify(canonical, null, 2));
  }

  return (
    <Box sx={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 2, p: 2, bgcolor: 'background.default', color: 'text.primary' }}>
      <Box component="aside" sx={{ bgcolor: 'background.paper', p: 2, borderRight: '1px solid', borderColor: 'divider' }}>
        <Typography variant="h6" gutterBottom>APIs</Typography>
        <Box component="nav">
          <Typography sx={{ fontWeight: 700, mb: 1 }}>MyAPI</Typography>
          <Typography component={RouterLink} to={`/apis/${id}/routes`} sx={{ display: 'block', color: 'primary.main', textDecoration: 'none', mb: 0.5 }}>
            Resources
          </Typography>
          <Typography sx={{ fontWeight: 700 }}>Methods ({SAMPLE.length})</Typography>
          <Typography>Settings</Typography>
        </Box>
      </Box>

      <Box component="main" sx={{ bgcolor: 'background.paper', p: 2, border: '1px solid', borderColor: 'divider' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="h5">Routes</Typography>
          <Box>
            <Button variant="contained" color="primary" onClick={handleCreate} sx={{ mr: 1 }}>Create Method</Button>
        ...
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', my: 2 }}>
          ...
          <Select size="small" value={String(showCount)} onChange={(e) => setShowCount(Number(e.target.value))} sx={{ minWidth: 80 }}>
            <MenuItem value="10">10</MenuItem>
            <MenuItem value="25">25</MenuItem>
            <MenuItem value="50">50</MenuItem>
          </Select>

          <TextField placeholder="Search routes..." size="small" value={filter} onChange={(e) => setFilter(e.target.value)} sx={{ ml: 'auto', minWidth: 240, bgcolor: 'background.default' }} />

          <Select size="small" value={configMode} onChange={(e) => setConfigMode(e.target.value as any)} sx={{ ml: 2 }}>
          ...
          <FormControlLabel control={<Checkbox checked={isGraphQL} onChange={(e) => setIsGraphQL(e.target.checked)} />} label="GraphQL" sx={{ ml: 1 }} />
        </Box>

        {isGraphQL ? (
          <Box>
            <Typography variant="subtitle1">GraphQL Schema (SDL)</Typography>
            <TextField fullWidth multiline minRows={6} value={jsonText} onChange={(e) => {
              setJsonText(e.target.value);
              const v = e.target.value;
              const errs: string[] = [];
              if (!/type\s+Query/.test(v)) errs.push('SDL: missing Query type');
              setValidationErrors(errs);
              if (errs.length === 0) setCanonical({ apiVersion: 'v1', name: id || 'graphql-api', type: 'GRAPHQL', sdl: v });
            }} sx={{ mt: 1 }} />
          </Box>
        ) : (
          <>
                ...
                <Table size="small">
                  ...
                  <TableBody>
                    {rowsData !== null && rowsData.length === 0 ? (
                      <TableRow><TableCell colSpan={8}><Typography>No APIs</Typography></TableCell></TableRow>
                    ) : (
                      rows.map((r, i) => (
                        <TableRow key={i}>
                          <TableCell>{r.resource}</TableCell>
                          <TableCell><Chip label={r.method} color="primary" size="small" /></TableCell>
                          <TableCell>{r.auth}</TableCell>
                          <TableCell>{r.integration}</TableCell>
                          <TableCell>Disabled</TableCell>
                          <TableCell>Disabled</TableCell>
                          <TableCell><Button variant="text" onClick={() => handleEdit(r, i)} sx={{ color: 'warning.main' }}>Edit</Button></TableCell>
            ...
            {configMode === 'json' && (
              <Box sx={{ mt: 1 }}>
                ...
                <Box sx={{ display: 'flex', gap: 2, mt: 1 }}>
                  <TextField multiline minRows={10} value={jsonText} onChange={(e) => onJsonChange(e.target.value)} sx={{ flex: 1 }} />
                  <TextField multiline minRows={10} value={yamlText} sx={{ flex: 1 }} InputProps={{ readOnly: true }} />
                </Box>
              </Box>
            )}
            ...
            {configMode === 'terraform' && (
              <Box sx={{ mt: 1 }}>
                <Typography variant="subtitle1">Terraform Editor (preview)</Typography>
                <TextField multiline minRows={10} value={tfText} onChange={(e) => setTfText(e.target.value)} sx={{ width: '100%', mt: 1 }} />
                <Box sx={{ mt: 1 }}>
                  <Button variant="contained" onClick={applyTerraform}>Preview Plan</Button>
                </Box>
              </Box>
            )}
          </>
        ...
        <Box sx={{ mt: 2 }}>
          <Typography variant="body2">
            {rowsData === null ? (error ? `Showing ${rows.length} (fallback) — ${error}` : `Loading... showing ${rows.length}`) : (`Showing 1 to ${rows.length} of ${rowsData.length} entries`)}
          </Typography>
        </Box>
      </Box>
    </Box>
